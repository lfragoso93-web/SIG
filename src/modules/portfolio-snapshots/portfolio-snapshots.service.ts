...
export async function generateSnapshot(referenceDate: Date) {
  ...
  // Inicializa agregação por classe já com todas as classes ativas
  ...
  for (const assetClass of assetClasses) {
    byClass.set(assetClass.id, {
      assetClassId: assetClass.id,
      invested: 0,
      marketValue: 0,
      targetPercentage:
        assetClass.targetPercentage !== null && assetClass.targetPercentage !== undefined
          ? toNum(assetClass.targetPercentage)
          : null,
    });
  }
  ...
  const classSnapshots: Prisma.PortfolioClassSnapshotCreateManySnapshotInput[] = [
    ...
    targetPercentage:
      cls.targetPercentage !== null
        ? new Prisma.Decimal(cls.targetPercentage)
        : null,
  }));
...
function formatSnapshot(s: any) {
  return {
    ...
    byClass: s.classSnapshots.map((c: any) => ({
      ...
      targetPercentage:
        c.targetPercentage !== null && c.targetPercentage !== undefined
          ? toNum(c.targetPercentage)
          : null,
    })),
  };
}