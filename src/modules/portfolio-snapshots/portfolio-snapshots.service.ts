...
function toFriday(date: Date): Date {
  ...
  const diff = day <= 5 ? 5 - day : 5 - day + 7; // dias até a próxima sexta (0 se já for sexta)
...
export async function generateSnapshot(referenceDate: Date) {
  ...
  // Agrega por classe para o classSnapshot
  const byClass = new Map<string, { assetClassId: string; invested: number; marketValue: number }>();

  for (const asset of assets) {
    // Transações até a sexta
    ...
    // Preço de fechamento mais próximo da sexta
    ...
    // Proventos recebidos até a sexta
    ...
    assetSnapshots.push({
      ...
      profitLossPct:  investedAmount > 0
        ? new Prisma.Decimal(profitLoss / investedAmount)
        : null,
      incomeReceived: new Prisma.Decimal(incomeReceived),
    });

    // Agrega por classe
    const cls = byClass.get(asset.assetClassId) ?? { assetClassId: asset.assetClassId, invested: 0, marketValue: 0 };
  ...
  const classSnapshots: Prisma.PortfolioClassSnapshotCreateManySnapshotInput[] = [
    ...
    currentPercentage: totalMarketValue > 0
      ? new Prisma.Decimal(cls.marketValue / totalMarketValue)
      : null,
  }));

  // Upsert do snapshot principal
  const snapshot = await prisma.portfolioSnapshot.upsert({
    ...
    include: {
      ...
      classSnapshots: { include: { assetClass: { select: { code: true } } } },
    },
  });
